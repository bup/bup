...
WVPASS bup cat-file "src/latest/$(pwd -P)/src/foo" > cat-foo
WVPASS diff -u src/foo cat-foo

WVSTART "cat-file --meta"
WVPASS bup meta --create --no-paths src/foo > src-foo.meta
WVPASS bup cat-file --meta "src/latest/$(pwd -P)/src/foo" > cat-foo.meta
...
WVPASS bup cat-file --bupm "src/latest/$(pwd -P)/src/" > bup-cat-bupm
src_hash=$(WVPASS bup ls -s "src/latest/$(pwd -P)" | cut -d' ' -f 1) || exit $?
